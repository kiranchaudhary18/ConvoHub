...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Mobile Sidebar - Full screen when no active chat */}
      <div className={`md:hidden ${activeChat ? 'hidden' : 'flex'} flex-col w-full h-full bg-gray-50 dark:bg-gray-800`}>
          {/* Header */}
          ...
          <div className="flex gap-1 p-2 border-b border-gray-200 dark:border-gray-700">
            {['chats', 'users', 'groups'].map((tab) => (
              <button
                ...
                className={`flex-1 py-2 px-2 rounded-lg font-medium transition capitalize text-sm ${
                  activeTab === tab
      ...
      {/* Desktop Sidebar */}
      <div className={`hidden md:flex md:flex-col ${sidebarOpen ? 'md:w-80' : 'md:w-20'} bg-gray-50 dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 transition-all duration-300`}
      >
        {/* Header */}
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between mb-4">
        ...
        {/* Tabs */}
        {sidebarOpen && (
          <div className="flex gap-1 md:gap-2 p-2 md:p-4 border-b border-gray-200 dark:border-gray-700">
            {['chats', 'users', 'groups'].map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`flex-1 py-2 px-2 md:px-4 rounded-lg font-medium transition capitalize text-sm md:text-base ${
                  activeTab === tab
                    ? 'bg-gradient-to-r from-pink-600 to-purple-600 text-white'
                    : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'
                }`}
